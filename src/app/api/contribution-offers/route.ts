...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const ctx = await buildRouteContext(req, db, "api:contribution-offers");
    requireMemberRoute(ctx.actorRole);

    const service = new RuntimeApiService(db);
    const result = await executeWithErrorHandling(ctx, () => service.createContributionOffer(ctx, payload));
