...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const ctx = await buildRouteContext(req, db, "api:recipient-requests");
    requireMemberRoute(ctx.actorRole);

    const service = new RuntimeApiService(db);
    const result = await executeWithErrorHandling(ctx, () => service.createRecipientRequest(ctx, payload));
