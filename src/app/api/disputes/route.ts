...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const ctx = await buildRouteContext(req, db, "api:disputes:open");

    const service = new RuntimeApiService(db);
    const result = await executeWithErrorHandling(ctx, () => service.openDispute(ctx, payload));
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    const payload = await req.json();
    const ctx = await buildRouteContext(req, db, "api:disputes:resolve");

    const service = new RuntimeApiService(db);
    const result = await executeWithErrorHandling(ctx, () => service.resolveDispute(ctx, payload));
    return NextResponse.json(result, { status: mapApiResultStatus(result) });
  } catch (error) {
    return routeErrorResponse(error, fallbackCorrelationId);
  }
