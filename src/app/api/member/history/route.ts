...
import { buildRouteContext, requireReadOnlyHistoryRoute, routeErrorResponse } from "@/lib/route-auth";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const ctx = await buildRouteContext(req, db, "api:member:history");
    requireReadOnlyHistoryRoute(ctx.actorRole);

    const service = new RuntimeApiService(db);
    const result = await executeWithErrorHandling(ctx, () => service.viewMemberStatusHistory(ctx));
