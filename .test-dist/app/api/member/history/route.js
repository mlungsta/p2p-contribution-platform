...
async function GET(req) {
    ...
    const limited = await (0, api_route_1.enforceRateLimit)(req, "public", fallbackCorrelationId);
    if (limited)
        return limited;
    try {
        const ctx = await (0, route_auth_1.buildRouteContext)(req, db_1.db, "api:member:history");
        (0, route_auth_1.requireReadOnlyHistoryRoute)(ctx.actorRole);
        const service = new runtime_api_service_1.RuntimeApiService(db_1.db);
        const result = await (0, runtime_api_service_1.executeWithErrorHandling)(ctx, () => service.viewMemberStatusHistory(ctx));
