...
async function GET(req) {
    ...
    if (limited)
        return limited;
    try {
        const ctx = await (0, route_auth_1.buildRouteContext)(req, db_1.db, "api:admin:override-context");
        const service = new runtime_api_service_1.RuntimeApiService(db_1.db);
        const result = await (0, runtime_api_service_1.executeWithErrorHandling)(ctx, () => service.getOverrideContext(ctx));
