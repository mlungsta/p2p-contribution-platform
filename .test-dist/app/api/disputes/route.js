...
async function POST(req) {
    ...
    const limited = await (0, api_route_1.enforceRateLimit)(req, "mutation", fallbackCorrelationId);
    if (limited)
        return limited;
    try {
        const payload = await req.json();
        const ctx = await (0, route_auth_1.buildRouteContext)(req, db_1.db, "api:disputes:open");
        const service = new runtime_api_service_1.RuntimeApiService(db_1.db);
        const result = await (0, runtime_api_service_1.executeWithErrorHandling)(ctx, () => service.openDispute(ctx, payload));
...
async function PATCH(req) {
    ...
    try {
        const payload = await req.json();
        const ctx = await (0, route_auth_1.buildRouteContext)(req, db_1.db, "api:disputes:resolve");
        const service = new runtime_api_service_1.RuntimeApiService(db_1.db);
        const result = await (0, runtime_api_service_1.executeWithErrorHandling)(ctx, () => service.resolveDispute(ctx, payload));
        return server_1.NextResponse.json(result, { status: (0, api_route_1.mapApiResultStatus)(result) });
    }
    catch (error) {
        return (0, route_auth_1.routeErrorResponse)(error, fallbackCorrelationId);
